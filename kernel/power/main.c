...
int cpufreq_max_limit_coupled = SCALING_MAX_UNDEFINED; /* not yet defined at startup */
...
/*		min_freq = policy->cpuinfo.min_freq;
		...
		min_freq = policy->min; /*   use govenor's min/max scaling to limit the freq table */
					...
					const char *buf, size_t n)
{
	...
	if (val == -1) { /* Unlock request */
		if (cpufreq_max_limit_val != -1) {
			...
			/* unlock now means set lock to scaling max to support powersave mode properly */
